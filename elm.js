...
Elm.Main.make = function (_elm) {
   ...
   var relativeAngle = F2(function (_v0,
   _v1) {
      return function () {
         switch (_v1.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 {case "_Tuple2":
                    return A2($Basics.atan2,
                      ...
                      _v1._0 - _v0._0);}
                 _U.badCase($moduleName,
                 "on line 88, column 5 to 35");
              }();}
         _U.badCase($moduleName,
         "on line 88, column 5 to 35");
      }();
   });
   var thrustVelocity = function (model) {
      return function () {
         var nhalfpi = (0 - $Basics.pi) / 2;
         var halfpi = $Basics.pi / 2;
         var angle = A2(relativeAngle,
         model.pos,
         model.target);
         return _U.cmp(angle,
         0) > 0 && _U.cmp(angle,
         halfpi) < 0 ? {ctor: "_Tuple2"
                       ,_0: angle / halfpi
                       ,_1: 1 - angle / halfpi} : _U.cmp(angle,
         halfpi) > 0 ? {ctor: "_Tuple2"
                       ,_0: (angle - halfpi) / halfpi
                       ,_1: 1 - (angle - halfpi) / halfpi} : _U.cmp(angle,
         0) < 0 && _U.cmp(angle,
         nhalfpi) > 0 ? {ctor: "_Tuple2"
                        ,_0: angle / halfpi
                        ,_1: 1 - angle / halfpi} : _U.cmp(angle,
         nhalfpi) < 0 ? {ctor: "_Tuple2"
                        ,_0: (angle + halfpi) / halfpi
                        ,_1: 1 - (angle + halfpi) / halfpi} : _U.badIf($moduleName,
   ...
   var decay = 0.12;
   var accel = 0.25;
   var velocityCalc = function (model) {
      return model.thrusting ? {ctor: "_Tuple2"
                               ,_0: accel * $Basics.fst(thrustVelocity(model))
                               ,_1: accel * $Basics.snd(thrustVelocity(model))} : {ctor: "_Tuple2"
                                                                                  ,_0: A2($Basics.max,
                                                                                  0,
      ...
      return function () {
         ...
         {case "Target":
            switch (action._0.ctor)
              {case "_Tuple2":
                 return function () {
                      var rmy = $Basics.toFloat($Basics.snd(model.viewport)) / 2.0 - $Basics.toFloat(action._0._1);
                      var rmx = $Basics.toFloat(action._0._0) - $Basics.toFloat($Basics.fst(model.viewport)) / 2.0;
                      return _U.replace([["target"
                                         ,{ctor: "_Tuple2"
                                          ,_0: rmx
                                          ,_1: rmy}]],
                      model);
            ...
            case "Tick":
            return function () {
                 var velocity = velocityCalc(model);
   ...
   var render = function (model) {
      ...
      _L.fromArray([$Graphics$Collage.move({ctor: "_Tuple2"
                                           ...
                                           ,_1: $Basics.snd(model.pos)})($Graphics$Collage.rotate(A2(relativeAngle,
                   model.pos,
                   model.target))($Graphics$Collage.filled($Color.green)(A2($Graphics$Collage.ngon,
                   3,
                   10))))
                   ,$Graphics$Collage.toForm($Graphics$Element.show(model))]));
   };
